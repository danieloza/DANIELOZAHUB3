...
def render_day_view(day_iso: str, employee_name: str) -> str:
    ...
    for v in visits_sorted:
        ...
        status = (v.get("status") or "planned").strip()
        ...
        lines.append(
            f"- {hhmm} | {client} | {service} ({dur}m) | {price_txt} | {status}"
        )
...
async def handle_day_callback(
    ...
    if data == "TODAY":
        ...
        await q.message.reply_text(
            "Select employee (today):", reply_markup=kb_day_employees(day_iso)
        )
        return True
    ...
    if data.startswith("D:ADD:"):
        ...
        if not hours:
            await q.message.reply_text(
                f"⛔ {emp} nie pracuje w tym dniu.",
                reply_markup=kb_day_actions(day_iso, emp),
            )
            return True
    ...
    if data.startswith("D:CANCEL_SELECT:"):
        ...
        if not visits:
            await q.message.reply_text(
                "No visits to cancel.", reply_markup=kb_day_actions(day_iso, emp)
            )
            return True
        await q.message.reply_text(
            "Select visit to cancel:",
            reply_markup=kb_day_cancel_list(day_iso, emp, visits),
        )
        return True
    ...
    if data.startswith("D:CANCEL_DO:"):
        ...
        try:
            ...
            await q.message.reply_text(
                "Could not cancel visit (API error).",
                reply_markup=kb_day_actions(day_iso, emp),
            )
        return True
    ...
    if data.startswith("D:MOVE_SELECT:"):
        ...
        if not visits:
            await q.message.reply_text(
                "No visits to move.", reply_markup=kb_day_actions(day_iso, emp)
            )
            return True
        await q.message.reply_text(
            "Select visit to move:", reply_markup=kb_day_move_list(day_iso, emp, visits)
        )
        return True
    ...
    if data.startswith("D:MOVE_PICK:"):
        ...
        if original_dt is None:
            original_dt = datetime.combine(
                date.fromisoformat(source_day), datetime.min.time()
            )
        ...
        await q.message.reply_text(
            ...
            reply_markup=kb_move_calendar(
                original_dt.year, original_dt.month, source_day, emp
            ),
        )
        return True
    ...
    if data.startswith("D:MOVE_MON_PREV:") or data.startswith("D:MOVE_MON_NEXT:"):
        ...
        target = (
            ...
            if data.startswith("D:MOVE_MON_PREV:")
    ...
    if data.startswith("D:MOVE_DATE:"):
        ...
        if not hours:
            await q.message.reply_text(
                ...
                reply_markup=kb_move_calendar(
                    date.fromisoformat(selected_day).year,
                    date.fromisoformat(selected_day).month,
                    source_day,
                    emp,
                ),
            )
    ...
    if data.startswith("D:MOVE_BACK_HOUR:"):
        ...
        if not hours:
            await q.message.reply_text(
                "Selected day is not available for this employee."
            )
            return True
        start_h, end_h = hours
        await q.message.reply_text(
    ...
    if data.startswith("D:MOVE_HOUR:"):
        ...
        await q.message.reply_text(
            "Select new minutes:",
            reply_markup=kb_move_minutes(
                int(hour), selected_day, source_day, emp, end_h
            ),
        )
        return True
    ...
    if data.startswith("D:MOVE_MIN:"):
        ...
        if not visit_id or hour is None:
            await q.message.reply_text(
                "Move state expired. Start again.",
                reply_markup=kb_day_actions(source_day, emp),
            )
            return True

        if not is_within_employee_hours(
            emp, int(hour), int(minute), DEFAULT_DURATION_MIN, selected_day
        ):
            hours = get_employee_hours(emp, selected_day)
            if not hours:
                await q.message.reply_text(
        ...
        start_dt = datetime.strptime(
            f"{selected_day} {int(hour):02d}:{int(minute):02d}", "%Y-%m-%d %H:%M"
        )
        ...
        try:
            ...
            await q.message.reply_text(
                "Visit moved.\n\n" + text,
                reply_markup=kb_day_actions(selected_day, emp),
            )
        except RequestException:
            await q.message.reply_text(
                "Could not move visit (API error).",
                reply_markup=kb_day_actions(source_day, emp),
            )
        return True
